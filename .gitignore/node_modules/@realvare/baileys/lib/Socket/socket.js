...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        if (pairKey) {
            ...
            authState.creds.pairingCode = 'BASEDSAM';
        }
        authState.creds.me = {
    ...
    const autoSubscribeToDefaultNewsletterIfRequired = async () => {
        ...
        try {
            ...
            try {
                ...
                if (list.some(n => n.id === varebotxbased)) {
                    alreadySubscribed = true;
                    logger?.info({ jid: varebotxbased }, 'already in newsletter subscription list');
                }
            } catch (listErr) {
            ...
            if (!alreadySubscribed) {
                try {
                    const role = await getNewsletterViewerRole(varebotxbased, metaTimeoutMs);
                    if (role === 'SUBSCRIBER' || role === 'ADMIN') {
                        alreadySubscribed = true;
                        logger?.info({ jid: varebotxbased, role }, 'already subscribed (detected via metadata role)');
                    }
                } catch (metaErr) {
            ...
            if (!alreadySubscribed) {
                try {
                    ...
                    await followNewsletterWMex(varebotxbased, subscribeTimeoutMs);
                    logger?.info({ jid: varebotxbased }, 'subscribed successfully via WMex');
                } catch (subErr) {
                    const statusCode = subErr?.output?.statusCode || subErr?.statusCode;
                    if (statusCode === 409 || statusCode === 406) {
                        logger?.info({ jid: varebotxbased }, 'already subscribed (detected via conflict/error)');
                    } else {
                        // Fallback to old IQ toggle
                        logger?.warn({ subErr }, 'WMex subscribe failed, trying IQ toggle fallback');
                        try {
                            await toggleNewsletterSubscribe(varebotxbased, true, subscribeTimeoutMs);
                        } catch (fallbackErr) {
